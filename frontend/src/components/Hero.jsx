...
function LightParticles({ reduceMotion }) {
  ...
  return (
    <div className="hero-particles" aria-hidden="true">
      {particles.map((_, i) => {
        const size = Math.random() * 1.4 + 0.8; // tiny: 0.8px to 2.2px
        const left = `${Math.random() * 100}%`;
        const duration = Math.random() * 30 + 30; // very slow: 30s to 60s
        const delay = Math.random() * -30; // start immediately in different phases
        return (
          <motion.span
            key={i}
            ...
            animate={{
              ...
              opacity: [0, 0.08, 0.18, 0.08, 0], // subtle opacity
            }}
            transition={{
              duration,
              repeat: Infinity,
          ...
          {ready && (
            <>
              ...
              <motion.div className="actions" {...fadeUp(0.72)}>
                ...
                <Link className="secondary-link" to="/custom-order">
